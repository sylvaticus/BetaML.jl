...
# MLJ interface for the Utils models

import MLJModelInterface       # It seems that having done this in the top module is not enought
const MMI = MLJModelInterface  # We need to repeat it here

# ------------------------------------------------------------------------------
# Start AutoEncoderMLJ

export AutoEncoderMLJ
...
```
"""
Base.@kwdef mutable struct AutoEncoderMLJ <: MMI.Deterministic
...
"""
function MMI.fit(m::AutoEncoderMLJ, verbosity, X)
    ...
    mi = AutoEncoder(;e_layers=m.e_layers,d_layers=m.d_layers,outdims=m.outdims,innerdims=m.innerdims,loss=m.loss, dloss=m.dloss, epochs=m.epochs, batch_size=m.batch_size, opt_alg=m.opt_alg,shuffle=m.shuffle, tunemethod=m.tunemethod, cache=false, descr=m.descr, rng=m.rng, verbosity=verbosity)
    fit!(mi,x)
    fitresults = mi
    cache      = nothing
    report     = nothing
 ...
 MMI.predict(m::AutoEncoderMLJ, fitresult, Xnew) = predict(fitresult, MMI.matrix(Xnew))

 MMI.transform(m::AutoEncoderMLJ, fitresult, Xnew) = MMI.predict(m::AutoEncoderMLJ, fitresult, Xnew)

 MMI.inverse_transform(m::AutoEncoderMLJ, fitresult, Xnew) = inverse_predict(fitresult, MMI.matrix(Xnew))


 MMI.metadata_model(AutoEncoderMLJ,
