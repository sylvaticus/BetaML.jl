...
PerceptronClassifier(;
  ...
  ) = PerceptronClassifier(initialθ,initialθ₀,maxEpochs,shuffle,forceOrigin,returnMeanHyperplane)
...
#kernelPerceptron(x, y; K=radialKernel, T=100, α=zeros(Int64,length(y)), nMsgs=0, shuffle=false)
 ...
 KernelPerceptronClassifier(;
    ...
    initialα = Int64[],
    shuffle=false,
    ) = KernelPerceptronClassifier(K,maxEpochs,initialα,shuffle)
...
PegasosClassifier(;
  ...
  forceOrigin=false,
  returnMeanHyperplane=false,
  ) = PegasosClassifier(initialθ,initialθ₀,λ,η,maxEpochs,shuffle,forceOrigin,returnMeanHyperplane)
...
function MMI.fit(model::PerceptronClassifier, verbosity, X, y)
 ...
 fitresult = perceptron(x, y; θ=initialθ, θ₀=model.initialθ₀, T=model.maxEpochs, nMsgs=0, shuffle=model.shuffle, forceOrigin=model.forceOrigin, returnMeanHyperplane=model.returnMeanHyperplane)
 cache=nothing
 report=nothing
...
function MMI.fit(model::KernelPerceptronClassifier, verbosity, X, y)
 ...
 fitresult = kernelPerceptron(x, y; K=model.K, T=model.maxEpochs, α=initialα, nMsgs=0, shuffle=model.shuffle)
 cache=nothing
 report=nothing
 return fitresult, cache, report
end
...
function MMI.fit(model::PegasosClassifier, verbosity, X, y)
 x = MMI.matrix(X)                     # convert table to matrix
 initialθ  = length(model.initialθ) == 0 ? zeros(size(x,2)) : model.initialθ
 fitresult = pegasos(x, y; θ=initialθ,θ₀=model.initialθ₀, λ=model.λ,η=model.η, T=model.maxEpochs, nMsgs=0, shuffle=model.shuffle, forceOrigin=model.forceOrigin, returnMeanHyperplane=model.returnMeanHyperplane)
 cache=nothing
 report=nothing
 return fitresult, cache, report
end
