...
#src end
# We fix the random seed for Flux, altough you may still get different results depending on the number of threads used.. this is a problem we solve in BetaML with creating a masterseed and then reseeding with it each parallel computation.
Random.seed!(seed)
