...
x = [0.1,1]
y = [1,0]
l1   = DenseNoBiasLayer(2,2,w=[2 1;1 1],f=identity,rng=FIXEDRNG)
...
train!(mynn,x',y',epochs=10000,batchSize=1,sequential=true,verbosity=NONE,optAlg=SGD(η=t->η,λ=1),rng=FIXEDRNG)
...
li   = DenseLayer(2,2,w=[2 1;1 1],f=identity,rng=FIXEDRNG)
...
l1 = DenseLayer(2,3,w=[1 1; 1 1; 1 1], wb=[0 0 0], f=tanh, df=dtanh,rng=FIXEDRNG)
l2 = DenseNoBiasLayer(3,2, w=[1 1 1; 1 1 1], f=relu, df=drelu,rng=FIXEDRNG)
l3 = DenseLayer(2,1, w=[1 1], wb=[0], f=identity,df=didentity,rng=FIXEDRNG)
mynn = buildNetwork([l1,l2,l3],squaredCost,name="Feed-forward Neural Network Model 1",dcf=dSquaredCost)
train!(mynn,xtrain,ytrain,batchSize=1,sequential=true,epochs=100,verbosity=NONE,optAlg=SGD(η=t -> 1/(1+t),λ=1),rng=FIXEDRNG)
...
train!(mynn,xtrain,ytrain,batchSize=1,sequential=true,epochs=100,verbosity=NONE,optAlg=ADAM(),rng=FIXEDRNG)
avgLoss = loss(mynn,xtest,ytest)
...
l1   = DenseLayer(2,3,w=ones(3,2), wb=zeros(3),rng=FIXEDRNG)
l2   = DenseLayer(3,1, w=ones(1,3), wb=zeros(1),rng=FIXEDRNG)
mynn = buildNetwork([l1,l2],squaredCost,name="Feed-forward Neural Network Model 1")
train!(mynn,xtrain,ytrain,epochs=1000,sequential=true,batchSize=1,verbosity=NONE,optAlg=SGD(η=t->0.01,λ=1),rng=FIXEDRNG)
avgLoss = loss(mynn,xtest,ytest)
...
l1   = DenseLayer(4,10, w=ones(10,4), wb=zeros(10),f=celu,rng=FIXEDRNG)
l2   = DenseLayer(10,3, w=ones(3,10), wb=zeros(3),rng=FIXEDRNG)
...
train!(mynn,xtrain,ytrain_oh,epochs=254,batchSize=8,sequential=true,verbosity=NONE,optAlg=SGD(η=t->0.001,λ=1),rng=FIXEDRNG)
...
l3   = VectorFunctionLayer(3,3,f=softmax)
mynn = buildNetwork([l1,l2,l3],squaredCost,name="Multinomial logistic regression Model Sepal")
train!(mynn,xtrain,ytrain_oh,epochs=10,batchSize=8,sequential=true,verbosity=NONE,optAlg=ADAM(η=t -> 1/(1+t), λ=0.5),rng=FIXEDRNG)
ŷtrain = Nn.predict(mynn,xtrain)
ŷtest  = Nn.predict(mynn,xtest)
trainAccuracy = accuracy(ŷtrain,ytrain,tol=1)
testAccuracy  = accuracy(ŷtest,ytest,tol=1)
